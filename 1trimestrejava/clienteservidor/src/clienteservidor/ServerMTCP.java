...
public class ServerMTCP{
    ...
    public static void main(String args[]){
        ...
        try{
            ...
            System.out.println("Servidor Eliza iniciando en el puerto: " + serverPort);
            // CICLO DEL SERVIDOR 
            while (true){
...
class Connection extends Thread{
   ...
   private static final String RESPUESTA_POR_DEFECTO = "Lo siento, no te comprendo.";

   static {
	   RESPUESTAS.put("HOLA", "Hola, ¿qué tal?");
	   RESPUESTAS.put("ENCANTADO", "Encantado de conocerte yo también");
       RESPUESTAS.put("ADIOS", "Adiós, espero volverte a ver pronto");
       RESPUESTAS.put("HORA", "Lo siento, no llevo reloj");
       RESPUESTAS.put("NOMBRE", "Mi nombre es Eliza");
       RESPUESTAS.put("CACA", "Creo que tu lenguaje no es adecuado");
   }

   public static String procesarMensaje ( String cadena) {
   	if(cadena == null) return RESPUESTA_POR_DEFECTO;
  //Convertire a mayusculas para comparar las palabras clave sin importar como esten escritas..
  //.trim quita el spacios innecesarios al principio y final del texto y touppercase convierte a mayusculas lo que escribe el usuario.
   String textoUpper = cadena.trim().toUpperCase();

   for (Map.Entry<String, String> entradaMapa : RESPUESTAS.entrySet()) {
	   if (textoUpper.contains(entradaMapa.getKey())) {

			   return entradaMapa.getValue();
		   }
	   }
   ...
   public Connection(Socket aSocket){
	   ...
	   // Establece canal de salidaº		º	1wsxZ	º1	qx
   ...
   public void run(){
	   ...
	   if (msg.trim().equalsIgnoreCase("Adios")) {
